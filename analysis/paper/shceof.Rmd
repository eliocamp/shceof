...
We restrict this paper to the spring September-October-November (SON) trimester. This season experiences a maximum in tropical teleconnections [@cazes-boezio2003a] and stratosphere-troposphere interactions [@lim2018].
...
Sea Surface Temperature (SST) monthly fields are from Extended Reconstructed Sea Surface Temperature (ERSST) v5 [@huang2017] and precipitation monthly data from the CPC Merged Analysis of Precipitation [@cmap], with a 2\degree and 2.5\degree resolution in latitude and longitude respectively.
...
Similarly, we chose the phase of cEOF2 so that the coefficient of determination between the Oceanic Niño Index [@bamston1997] and the real part is minimized, which also nearly maximizes the correlation with the imaginary part.
...
These atmospheric response is consistent with the effect of ENSO and the IOD on the extratropics: the SST anomalies initiate anomalous tropical convection that excites Rossby waves that propagate meridionally towards higher latitudes  [@mo2000; @cai2011; @nuncio2015].
...
The 135º phase is associated with SST in the central Pacific (Fig.\ \@ref(fig:sst-psi-2)a.1), while the 45º phase is associated with SST in the eastern Pacific (Fig.\ \@ref(fig:sst-psi-2)c.1), which correspond roughly to the Central Pacific and Eastern Pacific "flavours" of ENSO, respectively [@kao2009].
...
This mirrors the results of @cai2002, who showed that the CSIRO Model can create PSA-like variability even in the absence of ENSO forcing (i.e. with a climatological run), but that the variability of one of the PSA modes was enhanced when adding the ENSO signal.
...
Some results of our work contrast with those obtained by @irving2016 when analysing the PSA.
...
Moreover, @irving2016 use of ERA5 forectasted precipitation instead of the more observationally constrained CMAP dataset might also be an issue.
Additionally, the disagreement could be related to the fact that @irving2016 show weak relationship between ENSO and their PSA pattern, which explains partly the relationship between Australian rainfall and PSA-like variability.
...
The cEOF2 represents a wave-3 pattern with maximum magnitude in the Pacific sector, that is an alternative representation of the PSA1 and PSA2 patterns [@mo2001].
