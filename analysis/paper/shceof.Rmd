...
For real EOFs, this constant can be either 0 or $\pi$, corresponding to a change in sign. On the other hand, for cEOFs, it can be any real number between 0 and $2\pi$ [@horel1984], corresponding to rotations in the complex plane.
...
Most studies rely on correlations between an ENSO index and the SAM index [e.g. @lheureux2006, @cai2011a] or between the SAM index and other variables associated with tropical convection, such as OLR or tropical SSTS [e.g. @carvalho2005]. 
...
Therefore, the correlation between ENSO and SAM is predominantly the correlation between ENSO and PSA1 (the partial correlation between the SAM index and the ONI controlling for the 90º cEOF2 is `r partial_cor_enso[term == "sam", scales::number(partial_correlation, accuracy = 0.01)]`, p-value = `r partial_cor_enso[term == "sam", scales::pvalue(p.value, accuracy = 0.01)]`).
...
@irving2016 argued that there is some disagreement in the literature of whether the phase of the PSA pattern is affected by the location of tropical SS   
