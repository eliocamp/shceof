...
The large-scale extratropical circulation in the Southern Hemisphere (SH) is strongly zonally symmetric, but its zonal departures are highly relevant for regiona limpacts and could even be related to the occurrence of high-impact climate extremes (REFERENCE). 
...
Well-known features of the zonally asymmetric circulation in the SH are the Pacific–South American Patterns [@mo2001], which are associated with Rossby wave trains influencing precipitation anomalies in regions like South America, particularly in the September-October-November trimester (cita). 
...
@irving2015 showed that planetary wave amplitude in general is associated to sea-ice concentration, as well as to temperature and precipitation anomalies {where?}.

Some of these zonally asymmetric patterns of the SH circulation variability, appear to have experienced secular changes. 
...
Zonally asymmetric circulation is typically described by the amplitude and phase of zonal waves obtained by Fourier decomposition of geopotential heights or sea-level pressures at each latitude  [e.g. @vanloon1972; @trenberth1980; @turner2017].
This approach suggests that zonal waves 1 and 3 explain most of the variance of the SH tropospheric extratropical circulation (@vanloon1972). 
However, this methodology relies on the assumption that the circulation can be meaningfully described in terms of zonal waves of constant amplitude along a latitude circle. 
However, this is not valid for meridionally propagating waves or zonal waves with localised amplitudes.
In the case of the wave 3, for example, @trenberth1985 concluded that it plays a role in the development of blocking events, but this was mostly due to increased amplitude of a longitudinally localised wave train instead of an hemispheric-scale zonal wave 3.


The Fourier technique can be generalised to integrate all planetary wave amplitude? regardless of wave number, by computing the wave envelope @irving2015.
...
Another extensively used approach to characterize the SH circulation anomalies, is by computing Empirical Orthogonal Functions (EOF, also known as Principal Component Analysis). 
Within the EOF framework, the Southern Annular Mode (SAM) appears as the leading mode of variability of the SH circulation [@fogt2020] followed by two PSA-like patterns (Mo and Paegle 2001). 
...
Moreover, the 2nd and 3rd EOFs, usually known as PSA1 and PSA2 patterns, respectively, describe meridionally propagating wave trains that originates in the eastern equatorial Pacific and Australian-Indian Ocean sector, and travel towards the South Atlantic following a great-circle arch along the Antarctic coast. 
These patterns are usually derived by applying EOF to temporal anomalies, but @raphael2003 also applied EOF methods specifically to zonal anomalies.
...
@irving2016 characterise this pattern in a Fourier framework. Being a meridionally propagating mode, the PSA cannot be correctly characterised by Fourier decomposition at each latitude circle.
As a result, they reprojected meridional wind fields so that the path of the PSA laid on the equator. On this new projection, they could identify the PSA using Fourier decomposition.

Finally, another, hybrid methodology commonly used consists on identifying particular features of interest and creating indices using simple methods such as averages and differences.
The SAM Index defined by [@marshall2003] following Gong and @gong1999, for example, describes SAM variability by the averaging the difference of sea level pressure between 40ºS and 65ºS.
On the other hand, @raphael2004 used a similar methodology to describe the wave 3 activity in the SH circulation. 
Averaging standardised geopotential-height anomalies at three locations of the ridges associated to the zonal wave 3 of the climatological mean. 
By construction, the index is equivalent to projects the amplitude of the zonal wave 3 on the phase of the zonal wave 3 of the climatological mean field. 
Similarly, @hobbs2010 propose that the zonally asymmetric circulation in the SH can be described by the strength and location of two anticyclones located in the sub-Antarctic western hemisphere. 
...
Using information at specific regions or locations makes easy the computation of indices like those, but they are not very robust or unable to capture non-stationary patterns.


The general goal of this study is to improve the description and understanding of planetary waves in the SH using an index that is able to capture the phase-varying nature of planetary waves and is flexible enough to describe planetary waves with non-constant amplitude in each latitude circle. 
...
We apply this method to characterise the springtime zonally asymmetric circulation of the extratropical SH circulation. 
In addition to take vertical distribution into account, we analyse both the troposphere and the stratosphere.
