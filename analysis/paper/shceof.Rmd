...
To that end, we computed the coefficient of determination between the cEOFs time series and the three SAM indices (SAM, A-SAM and S-SAM) defined by @campitelli2022 at each vertical level (Fig.\ \@ref(fig:sam-eof-vertical)).
...
Such extremely high correlation between A-SAM and Imaginary cEOF2 suggests that the methodology proposed in this work is able to characterise the zonally asymmetric component of the SAM described previously by @campitelli2022.
...
Besides the Pacific ENSO-like pattern, there are positive anomalies in the western Indian Ocean and negative values in the eastern Indian Ocean, resembling a positive Indian Ocean Dipole [@saji1999].
...
These atmospheric response is consistent with the effect of ENSO and the Indian Ocean Dipole on the extratropics: the SST anomalies initiate anomalous tropical convection that excites Rossby waves that propagate meridionally towards higher latitudes  [@mo2000; @cai2011; @nuncio2015].
...
The 135º phase is associated with SST in the central Pacific (Fig.\ \@ref(fig:sst-psi-2)a.1), while the 45º phase is associated with SST in the eastern Pacific (Fig.\ \@ref(fig:sst-psi-2)c.1), which correspond roughly to the Central Pacific and Eastern Pacific and "flavours" of ENSO, respectively [@kao2009].
...
This mirrors the results of @cai2002, who showed that the CSIRO Model can create PSA-like variability even in the absence of ENSO forcing (with a climatological run), but that the variability of one of the PSA modes was enhanced when adding the ENSO signal.
...
Some aspects of our work contrast with @irving2016 analysis of the PSA.
