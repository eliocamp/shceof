...
We restrict this paper to the spring September-October-November (SON) trimester because during this season both the tropical teleconnections over South America [@cazes-boezio2003] and the SH stratosphere-troposphere interactions [@lim2018] are at their maximum amplitude. 
...
To describe the phase-varying nature of these two waves one could combine each pair into indices of amplitude of phase. 
So, for instance, the amplitude of the wave 1 EOF could be measured as $\sqrt{\mathrm{EOF1}^2 + \mathrm{EOF2}^2}$ and it's phase as $\tan^{-1} \left ( \frac{\mathrm{EOF2}}{\mathrm{EOF1}} \right )$ (where $\mathrm{EOF1}$ and $\mathrm{EOF2}$ are the time series associated with each EOF). 
However, this rests upon visual inspection of the spatial patterns and only works if both phases appear cleanly in different EOFs, which is not guaranteed. 
...
In Complex Empirical Orthogonal Functions (cEOF) analysis, each cEOF is represented by two spatial patterns that are shifted by 1/4 wavelength by construction. 
...
This is analogous to how any sine wave with any amplitude and phase can be constructed by the sum of a sine wave and cosine wave with different amplitude but constant phase. 
This means that cEOFs naturally represent phase-varying wave-like patterns that change location as well as amplitude. 

For example, Figure \@ref(fig:ceofs-1) a.1 shows the two spatial patterns of a cEOF, one with shaded  contours (which we call the 0º phase) and the other with black contours (which we call the 90º phase).
...
When the phase of the wave matches the 0º phase, then the 0º phase time series will be positive and the 90º phase time series will be zero. 
...
Cases will have non zero values in both time series will represent intermediate phases.

These two phases are not special and the same cEOF could be represented by any two linear combinations of the phases shown in Figure \@ref(fig:ceofs-1) a.1. 
...
Compute cEOFs are computed the same way as traditional EOFs except that the data is first augmented by computing its analytic signal. 
This is a complex number whose real part is the original series and the imaginary part is the original data shifted by 90\degree at each spectral frequency -- i.e. its Hilbert transform.
Each cEOF is a set of complex-valued spatial patterns and time series in which the real and imaginary parts are the two orthogonal phases of the wave-like pattern.
...
Therefore, we chose the phase of cEOF1 so that the time series corresponding to the 0ª cEOF1 has the maximum correlation with the zonal wave 1 of Total Column Ozone between `r knitr::combine_words(LatLabel(o3wave_lats))`.
Similarly, we chose the phase of cEOF2 so that the coefficient of determination between the Oceanic Niño Index [@bamston1997] and the º0 cEOF2 is minimised, which also nearly maximises the correlation with the 90º cEOF2.
...
This modulated amplitude is more evident at 200 hPa, which is consistent with the cEOF2 computed separatedly for 200 hPa explains a bit more variance than the cEOF2 computed separatedly for 50 hPa (`r scales::percent(ceof_splitted[, eof[[1]]$sdev, by = levs][cEOF == "cEOF2", r2][1])` vs. `r scales::percent(ceof_splitted[, eof[[1]]$sdev, by = levs][cEOF == "cEOF2", r2][2])`, respectively).
...
Comparing the location of the positive anomaly near 90ºW in column b of Figure \@ref(fig:eof2-regr-gh) with Figures 1.a and b from @mo2001, the 0º cEOF2 regression map could be identified with PSA2, while the 90º cEOF2 resembles PSA1.
...
As a result, the correlation between the 0º cEOF2 and ENSO is also not significant  (`r enso_cor[cEOF == "cEOF2" & part == "Real"]$text`).
...
This lack of strong relationship between the cEOF1 and temperature and precipitation might be surprising considering the correlation between the cEOF1 and the SAM (Fig. \@ref(fig:sam-eof-vertical) column 1) and the correlation between SAM and temperature east and west of the Antarctic Peninsula, and with precipitation in western Australia (BUSCAR CITA). 
There a two main reasons for this. 
First, the correlation between cEOF1 and the SAM in the troposphere is modest, with less than 50% of shared variance (Fig. \@ref(fig:sam-eof-vertical) column 1), so one need not expect these to indices to be equivalent. 
Second, on a previous paper [@campitelli2022] we showed that the strong relationship between the SAM and temperature anomalies around the Antarctic Peninsula is mainly due to the asymmetric part of the SAM. 
...
This allows us to show for example, that the 90º cEOF2, that resembles PSA1 variability, is closely associated to the SAM in the troposphere. 
