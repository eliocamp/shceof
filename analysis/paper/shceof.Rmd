...
pattern <- sams[term == "full" & lev == 200] %>% 
  .[era5[lev == 200], on = c("lev", "time")] %>% 
  .[, FitLm(hgt, estimate), by = .(lon, lat, lev)] %>% 
  rm_intercept() %>% 
  .[, estimate := mean(estimate), by = .(lat)] %>% 
  .[, term := NULL] %>% 
  setnames("estimate", "eof")

set.seed(42)


fake_sam <-  sams[term == "full" & lev == 200] %>% 
  copy() %>% 
  .[, value := sample(estimate, replace = TRUE)] %>% 
  .[, estimate := NULL] %>% 
  .[, term := NULL]
...
# fake_sam <-  sams[term == "full" & lev == 200] %>% 
#   copy() %>% 
#   .[, value := complement(estimate, y)] %>% 
#   .[, estimate := NULL] %>% 
#   .[, term := NULL]

cor_with_eof2 <- fake_sam[cEOF2, on = "time"] %>% 
  sep_ReIm() %>% 
  .[, correlate(value, hgt), by = part]

fake_sam <- fake_sam %>% 
  .[pattern, on = "lev", allow.cartesian = TRUE] %>% 
  .[, sam := value*eof]
...
var <- fake_eof$sdev %>%
  copy() %>% 
    .[, setNames(paste0(EOF, " (", scales::percent(r2), ")"),
                 EOF)]
var <- c(var, setNames(c("Annular Pattern", "Imaginary cEOF2", "Real cEOF2"), c("SAM", "Imaginary", "Real")))
