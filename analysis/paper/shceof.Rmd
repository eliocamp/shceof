...
```{r plot_sst_psi}
plot_sst_psi <- function(regr_sst, regr_psi, which_cEOF, which_lev) {
  ...
  stream_gdata %>% 
    ...
    geom_qmap(keep = .015) +
    scale_fill_divergent_discretised(latex2exp::TeX(r'(Streamfunction ($m^2s^{-1}\times 1e^{-7}$))'), 
                                     labels = one_less,
    ...
    coord_sf(ylim = c(-89, 0), expand = FALSE,
...
(ref:sst-psi-2-cap) Regression maps of cEOF2 with SST (K, column a) and streamfunction zonal anomalies ($m^2/s\times10^-7$, shaded) with their corresponding activity wave flux (vectors) (column b) for the 1979 -- 2019 period. Areas marked with dots have p-values smaller than 0.01 adjusted for FDR. 

```{r sst-psi-2,  fig.cap = "(ref:sst-psi-2-cap)", fig.width = 7, fig.height=4, dependson="plot_sst_psi", fig.env="figure*"}
plot_sst_psi(regr_sst = regr_sst, regr_psi = regr_psi,
             which_cEOF = "cEOF2", which_lev = 200) 
```
...
plot_sst_psi(regr_sst = regr_sst, regr_psi = regr_psi,
             which_cEOF = "cEOF1", which_lev = 200) 
```
...
plot_pp <- function(data, which_cEOF) {
  lats <- c(-55, -10)
  breaks <- AnchorBreaks(exclude = 0)(c(-.9, .9))

  data %>%
    # .[angle == unique(angle)[2]] %>% 
    .[lat %between% lats] %>% 
    # .[cEOF %in% which_cEOF] %>%
    # .[term %in% which_term] %>%
    .[, lev := "sfc"] %>%
    ...
    geom_contour_fill(aes(z = estimate, fill = ..level..),
                      breaks = breaks) +
    geom_contour_tanaka(aes(z = estimate), breaks = breaks) +
...
(ref:pp-oceania-cap) Regression of SON mean precipitation anomalies in Oceania (mm per day, shaded) with the (row a) real and (row b) imaginary pars of the (column 1) cEOF1 and (column 2) cEOF2. For the 1979 -- 2019. Areas marked with dots have p-values smaller than 0.01 adjusted for False Detection Rate. In black is the East Australia region, defined as the box between `r knitr::combine_words(LonLabel(box_australia$lon))` and `r knitr::combine_words(LatLabel(box_australia$lat))`.

```{r, fig.cap="precipitation correlation", fig.width=6.6, fig.height=5.5}
plot_pp(pp_regr, c("cEOF1", "cEOF2"))
```
...
# plot_pp(pp_regr, c("cEOF1", "cEOF2"))
# 
# (plot_sst_psi(regr_sst = sst_regr[angle == a], regr_psi = psi_regr[angle == a],
#               which_cEOF = "cEOF2", which_lev = 200)) / 
#   (
#     plot_pp(pp_regr[angle == a], c("cEOF1", "cEOF2"))
#   )
```
...
@cai2011 -> precipitación ENSO

 no vio relación con la pp. mezclan fases, pero la fase es crucial. 
