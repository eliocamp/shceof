...
This is analogous to the method of @irving2016, who used reprojection and Fourier filtering to detect "PSA-like" variability and defined as the proper PSA using the peaks of the phase distribution (Compare our Figure\ \@ref(fig:phase-histogram) with their Figure 6).
...
The Imaginary cEOF2 is related with the SAM through the A-SAM in the troposphere, with up to \textcolor{red}{(Acá aparece un -)} `r scales::percent(max_r2[cEOF == "cEOF2"]$estimate, 2)` of shared variance, reached at `r max_r2[cEOF == "cEOF2"]$lev` hPa (Figure\ \@ref(fig:sam-eof-vertical).b2). 
...
(ref:fake-eof-cap) EOFs of a synthetic data set of 41 times steps of geopotential height fields as the sum of the fields predicted by the cEOF2 plus perfectly zonally symmetric SAM-like anomalies with random amplitude.
