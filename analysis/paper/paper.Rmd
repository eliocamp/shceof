...
rotations_o3 %>%
  ...
  scale_x_continuous("Rotation", breaks = seq(-pi, pi, by = 60*pi/180),
...
(ref:t-vertical-cap) Regression of EOF1 with mean air temperature averaged between 80ºS and 25ºS for the period 1979 -- 2019.
...
t %>% 
  ...
  geom_contour_tanaka(data = ~.x[variable == "t"],
                      aes(z = estimate),
                      breaks = AnchorBreaks(0, binwidth = 0.25, exclude = 0)) +
  geom_contour2(data = ~.x[variable == "hgt"], 
                aes(z = estimate, linetype = factor(-sign(..level..))), 
                breaks = AnchorBreaks(0, exclude = 0)) +
  geom_text_contour(data = ~.x[variable == "hgt"], 
                aes(z = estimate), skip = 1, stroke = 0.15, stroke.colour = "white",
                breaks = AnchorBreaks(0, exclude = 0)) +
...
```{r o3-regr, fig.cap = "(raf:o3-regr-cap)", fig.height=4, fig.width = 5}
o3_regr %>% 
  periodic(lon = c(0, 360)) %>% 
...
Planetary waves that penetrate into the stratosphere are known to affect ozone distribution [@hartmann1979]. Figure \@ref(fig:o3-regr) shows regression maps of EOF1 with fields of Total Ozone Column (TOC). It shows zonal wave 1 pattens in TOC associated with both phases of EOF1. Climatologically, the springtime Ozone minimum is located off the South Pole and towards the Weddell Sea [@wirth1993]. Thus, the Real EOF1 regression pattern (Figure \@ref(fig:o3-regr)a) coincides with the climatological position of the ozone whole while the one for the Imaginary EOF1 is shifted by 90º. THe maximum TOC anomalies are co-located with the minimum temperature anomalies above 10 hPa and mix the maximum temperature anomalies below 10 hPa; that is, the TOC zonal wave 1 is anticorrelated with the temperature zonal wave 1 in the upper stratosphere, and directly correlated in the upper stratosphere. These phase differences are due to the change between the photochemically-dominated upper stratosphere, where cold temperatures inhibit the destruction of ozone, and the advectively-dominated lower stratosphere [@hartmann1979;@smith1995].  

This is explained by the inverse temperature dependence of the photochemical desctruction of ozone. 

This suggest that the EOF1 can be a complete description of the TOC zonal wave 1. 

Temperature anomalies in 

Both pars of EOF1 are associated with a temperature wave 1 pattern which changes phase around 10 hPa. This switch in the sign of the correlation in temperature at 10 hPa suggest a relationship with Ozone, since its indicative of the effects of planetary waves in the advection of Ozone [@hartmann1979;@rood1985;@smith1995]. 
...
Indeed, Figure \@ref(fig:o3-regr) shows the relationship between amplitude and phase of the planetary wave 1 in Total Ozone Column between 70ºS and 45ºS and amplitude and phase of the EOF1. The correlation between the zonal wave 1 of ozone and EOF1 (computed as the mean cosine of the difference in phase) is `r wave_correlation$text`. As expected from the location of anomalies in Figure \@ref(fig: o3-regr), the Real EOF1 drives the relationship with amplitude and the Imaginary EOF1 drives the relationship with phase (not shown). 
...
```{r sst-psi-2,  fig.cap = "(ref:sst-psi-2-cap)", fig.with = 6, fig.height=4}
...
```{r sst-psi-1, fig.cap = "(ref:sst-psi-1-cap)", fig.with = 6, fig.height=4}
