...
sam_r2 <- sams %>% 
  ...
  .[, with(cor.test(hgt, estimate), 
           list(estimate = estimate, 
                low = conf.int[1],
                high = conf.int[2],
                p_value = p.value)), by = .(lev, term, part, PC)] %>%
  
  .[, low2 := pmin(low^2, high^2, ifelse(sign(high) != sign(low), 0, 1))] %>% 
  # .[low2 <= 0, low2 := 0] %>% 
  .[, high2 := pmax(low^2, high^2)] %>% 
  .[PC != "PC3"]
...
sam_r2 %>% 
  ...
  geom_point(data = ~.x[p.adjust(p_value, "fdr") < 0.01], aes(color = term)) +
...
# +   annotate("rect", xmin = 170, xmax = 195, ymin = -25, ymax = -5, color = "black", fill = NA)
```

??? -\> esta intro hay que cambiarla. To help identify those sources Figure \@ref(fig:sst-psi-2) shows regression maps between EOF2 and STT and Streamfunction at 200 hPa. As expected from the correlations between each component of EOF2 and ENSO, Imaginary EOF2 is associated with strong ENSO-like SST anomalies and streamfunction (Figure \@ref(fig:sst-psi-2).a2). Streamfunction anomalies (Figure \@ref(fig:sst-psi-2).b2) are indicative of tropical forcing, which is compatible with SST anomalies. Real EOF2, on the other hand, is not associated with significant SST anomalies (Figure \@ref(fig:sst-psi-2).a1), as expected by its lack of relationship with ENSO, and streamfunction anomalies in the tropics are much weaker and non significant.
...
enso_cor <-  ceof[, eof[[1]]$left, by = .(season)] %>%
    ...
    enso[., on = "time"] %>%
    na.omit() %>% 
   .[, with(cor.test(hgt, oni), list(estimate = estimate,
                                     p.value = p.value,
                                     low = conf.int[1],
                                     hig = conf.int[2])), 
     by = .(PC, part)] %>% 
  .[, cor_text := paste0(signif(estimate, 2), " (CI: ", 
                         signif(low, 2), " -- ", signif(hig, 2), ")")]
...
ceof[, eof[[1]]$left, by = .(season)] %>% 
  ...
  geom_smooth(method = "lm", aes(weight = mag),
              formula = y ~ I(cos(x)) + I(sin(x)), 
              se = FALSE, color = "black", size = 0.5) +
  geom_point(aes(colour = mag > quantile(mag, 0.5)), size = 3) +
  scale_color_brewer(NULL, palette = "Dark2", 
...
The zonal wave represented by the EOF2 pattern is very similar to the wave train associated with ENSO. Indeed, the correlation between the Imaginary EOF2 and the Oceanic ENSO Index ([@bamston1997]) is `r enso_cor[PC == "PC2" & part == "Imaginary"]$cor_text`. The correlation between the Real EOF2 and ENSO is not significant (`r enso_cor[PC == "PC2" & part == "Real"]$cor_text`). This suggests that, even though both EOF2 patterns represent the same wave train with a different phase, these two phases are exited by different sources.

Figure \@ref(fig:sst-psi-2) suggests, then, that the wave train represented by EOF2 can be forced both by the tropics and internally, and that it has preferred phases in each case. When it is forced by the tropics, it aligns roughly with the Imaginary phase, and when it's the Real phase is closer to the internally-varying wave. However, Figure \@ref(fig:sst-psi-2).b1 also indicates that this separation is not completely clean. It's not expected.. ??
