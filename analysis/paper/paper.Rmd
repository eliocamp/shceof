...
geom_contour_pval <- function(mapping, p.value = 0.01, size = 0.1) {
  list(
    stat_contour_fill(mapping, breaks = c(0, p.value), fill = NA, geom = pattern_dots),
...
#   .[, FitLm(t, Real, Imaginary, se = TRUE), by = .(lev, lon, lat)] %>% 
#   rm_intercept()
# t_PC1_vertical %>% 
#   .[lev %in% c(1, 5, 10, 50)] %>% 
#   .[, p.value := Pvaluate(estimate, std.error, df, "fdr"), by = .(term, lev)] %>% 
#     ggplot(aes(lon, lat)) +
#   geom_contour_fill(aes(z = estimate)) +
#   stat_subset(aes(subset = p.value <= 0.01 & is.cross(lon, lat)),
#               size = .5, alpha = 0.5) +
#   geom_hline(yintercept = c(-80, -35)) +
#   geom_qmap(~.x[lat <= -20]) +
...
t %>% 
  ...
  geom_contour_pval(aes(z = p.value)) +
  scale_fill_divergent_discretised("Air temperature", guide = guide_colorsteps_bottom()) + 
  scale_y_level() +
  scale_x_longitude() +
  facet_grid(~term) +
  axis_labs_smol
...
The ozone hole, as measured by Total Ozone Column (TOC) is climatologically not centered over the South Pole, but shifted towards the Weddell Sea, which translates to a large amplitude zonal wave 1 with minimum over the Weddell Sea and maximum over the Davis Sea [@wirth1993]. The regression 
...
This suggests that the separation between the asymmetric and symmetric part of the SAM is not only statistically possible, but physically plausible. The asymmetric part of the SAM might be statistical contamination from one phase of the PSA mode. 
...
?? Compare with Figura 1 b and c (Streamfunction) and Figure 2 d and FIgure 3 d (SST) from [@mo2001] and Figure 7 [@irving2016].

Imaginary more global than real -> is that why it appears mixed with SAM?
